...
<!-- Left side column. contains the sidebar -->
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        <?=str_replace('_', ' ', $title);?>
        <small>Franchise Gym List</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="<?=site_url();?>"><i class="fa fa-dashboard"></i>Home</a></li>
        <!-- <li><a href="#" data-toggle='modal' data-target='#franchiseModal'><i class="fa fa-dashboard"></i>Register Franchise Gym Information</a></li> -->
      </ol>
    </section>
	...
	<div class="box">
            <div class="box-header">
              <h3 class="box-title">Franchise Gym Lists</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
...
<!-- register start -->
<div class="modal fade" id="franchiseModal" role="dialog">
      <div class="modal-dialog modal-sm">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-primary" id="btnaddfranchisegym">Submit</button>
            <button type="button" class="btn btn-success" id="btnupdatefranchisegym" style="display:none;">Update</button>
            <button type="button" class="btn btn-danger" id="btndeletefranchisegym" style="display:none;">Delete</button>
...
<script>
    $(document).ready(function(){
        ...
        loadBranchdetails();
        // loadBranchNames();
        $('#franchiseModal').on('shown.bs.modal', function (e) {
        // do something...
            action = $(e.relatedTarget).attr('data-action');
            
            console.log('user : ' + action)
            if(action === 'edit') {
                    $('#btnaddfranchisegym').hide();
                    $('#btnupdatefranchisegym').show();
                    $('#btndeletefranchisegym').hide();
                    $('#password').closest('.form-group').hide();
                    $('#password_confirmation').closest('.form-group').hide();
            }else if(action === 'delete'){
                    $('#btnupdatefranchisegym').hide();
                    $('#btndeletefranchisegym').show();
                    $('#password').closest('.form-group').hide();
                    $('#password_confirmation').closest('.form-group').hide();
            }else{
                $('#btnaddfranchisegym').show();
        ...
        $('#btnaddfranchisegym').click(regiterfranchiseGym);
        $('#btnupdatefranchisegym').click(updatefranchiseGym);
    });
    function regiterfranchiseGym(e){
        e.preventDefault();
        ...
        $.ajax({
            type:'POST',
            url: site_url + 'registerbranch/FranchiseGymInformation/insertBranchFromAjax' ,
            dataType:"json",
            data:formData,
            async: true,
            cache: false,
    ...
    function updatefranchiseGym(e){
        ...
        $.ajax({
            type:'POST',
            url: site_url + 'registerbranch/FranchiseGymInformation/updateBranchFromAjax' ,
            dataType:"json",
            data:formData,
            async: true,
            cache: false,
    ...
    function loadBranchdetails(){
        ...
        $('#branchListTable').DataTable({
            ...
            "ajax": {
                "url": site_url +"registerbranch/FranchiseGymInformation/ajaxLoadBranch" ,
                "type": "POST",
