  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="#" data-toggle='modal' data-target='#mainUserModal'><i class="fa fa-dashboard"></i>Register User Information</a></li>
      </ol>
    </section>
...
<div class="modal fade" id="mainUserModal" role="dialog">
      <div class="modal-dialog modal-sm">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-primary" id="btnaddmainuser">Submit</button>
            <button type="button" class="btn btn-success" id="btnupdatemainuser" style="display:none;">Update</button>
            <button type="button" class="btn btn-danger" id="btndeletemainuser" style="display:none;">Delete</button>
            <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
          </div>
        </div>
      </div>
...
<script>
    $(document).ready(function(){
        ...
        loadRole();
        loadPosition();
        $('#mainUserModal').on('shown.bs.modal', function (e) {
        // do something...
            ...
            if(action === 'edit') {
                    $('#btnaddmainuser').hide();
                    $('#btnupdatemainuser').show();
                    $('#btndeletemainuser').hide();
                    $('#password').closest('.form-group').hide();
                    $('#password_confirmation').closest('.form-group').hide();
            }else if(action === 'delete'){
                    $('#btnupdatemainuser').hide();
                    $('#btndeletemainuser').show();
                    $('#password').closest('.form-group').hide();
                    $('#password_confirmation').closest('.form-group').hide();
            }else{
                $('#btnaddmainuser').show();
        ...
        // });
        $('#btnaddmainuser').click(regiterMainLogin);
    ...
    function updateUser(e){
        ...
        $.ajax({
            type:'POST',
            url: site_url + 'registeruser/MainUserInformation/UpdateUserInformation' ,
            dataType:"json",
            data:formData,
            async: true,
            cache: false,
    ...
    function changeUserPassword(e){
        ...
        var password_confirmation = $(e.target).closest("div.modal-dialog").find('#password_confirmation1').val();
         ...
         $.ajax({
            type:'POST',
            url: base_url + 'registeruser/MainUserInformation/ChangeUserPassword' ,
            dataType:"json",
            data:{
                userID:userID,
                password:password,
    ...
    function loadUser(){
        ...
        $('#userListTable').DataTable({
            ...
            "ajax": {
                "url": site_url +"registeruser/MainUserInformation/ajaxLoadUser" ,
                "type": "POST",
                "dataSrc": function (data) {
                    ...
                    for (var i = 0; i < data.length; i++){
                        ...
                        data[i]["edit"] = "<button class='btn btn-info user_action' data-action='edit' data-obj='" + data_array + "' data-toggle='modal' data-target='#mainUserModal'><i class='fa fa-edit'></i> Edit</button>";
                        data[i]["delete"] = "<button class='btn btn-danger user_action' data-action='delete' data-obj='" + data_array + "' data-toggle='modal' data-target='#mainUserModal'><i class='fa fa-times'></i> Deactivate</button>";
                    }
    ...
    function loadBranchdetails(e){
        ...
        $.ajax({
            type:'POST',
            url: site_url +"registeruser/MainUserInformation/ajaxLoadBranch" ,
            dataType:"json",
            data:{
    ...
    function loadPosition(){
        ...
        $.ajax({
            type:'POST',
            url: site_url +"registeruser/MainUserInformation/ajaxLoadPosition" ,
            dataType:"json",
            success:function(data)
            {
