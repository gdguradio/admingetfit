...
<!-- Left side column. contains the sidebar -->
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        <?=str_replace('_', ' ', $title);?>
        <small>GymPhaselist</small>
      </h1>
      <ol class="breadcrumb">
      <li><a href="<?=site_url().''.'Welcome';?>"><i class="fa fa-dashboard"></i>Home</a></li>
        <li><a href="#" data-toggle='modal' data-target='#GymPhaseModal'><i class="fa fa-keyboard-o"></i>Register Gym Phase Information</a></li>      
      </ol>
    </section>
	...
	<div class="box">
            <div class="box-header">
              <h3 class="box-title">Gym Phase Lists</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <table id="gymphaseListTable" class="table table-bordered table-striped">
                <thead>
                <tr>
                    <th width="10">[#]</th>
  ...
  <!-- /.content-wrapper -->
<!-- start modal -->
<div class="modal fade" id="GymPhaseModal" role="dialog">
      <div class="modal-dialog modal-sm">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal">&times;</button>
            <h4 class="modal-title">Gym Phase</h4>
          </div>
          <div class="modal-body">
            <form role="form">
                ...
                <div class="box-body">
                    <input type="hidden" id="gymphaseID"/>
                    ...
                    <div class="form-group">
                      <label for="gymphasename">Gym Phase Name</label>
                      <input type="text" class="form-control" id="gymphasename" placeholder="Enter Gym Phase Name">
                    </div>
                    <div class="form-group">
                        <label for="">Has sub?</label>
                        <div class="custom-control custom-radio">
                            <input type="radio" class="custom-control-input" id="haschildno" name="haschild" value="no" checked>
                            <label class="custom-control-label" for="haschildno">No</label>
                        </div>
                        <div class="custom-control custom-radio">
                            <input type="radio" class="custom-control-input" id="haschildyes" name="haschild" value="yes" >
                            <label class="custom-control-label" for="haschildyes">Yes</label>
                        </div>
                    </div>
                    <div class="form-group">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-primary" id="btnaddgymphase">Submit</button>
            <button type="button" class="btn btn-success" id="btnupdategymphase" style="display:none;">Update</button>
            <button type="button" class="btn btn-danger" id="btndeletegymphase" style="display:none;">Delete</button>
...
<script>
    $(document).ready(function(){
        loadGymPhase();
        loadBranch();
        $('#GymPhaseModal').on('shown.bs.modal', function (e) {
        action = $(e.relatedTarget).attr('data-action');
        if(action === 'edit') {
                $('#btnupdategymphase').show();
                $('#btndeletegymphase').hide();
            }else if(action === 'delete'){
                $('#btnupdategymphase').hide();
                $('#btndeletegymphase').hide();
        }else{
            $('#btnaddgymphase').show();
            $('#btnupdategymphase').hide();
        ...
        $(document).on('click','.user_action',function(){
            ...
            $('#gymphaseID').val(obj.SysID);
            $('#gymphasename').val(obj.PhaseName);
            $('#description').val(obj.Description);
            $('#displayorder').val(obj.DisplayOrderIndex);
            $(':radio[name=haschild][value='+obj.HasSub.toLowerCase()+']').prop('checked','checked');
            ...
            // var imageurl = "<?php //echo site_url('assets/MainGymPhase/');?>";
            // var documenttype = obj.DocumentLink.substring(obj.DocumentLink.lastIndexOf('.')+1, obj.DocumentLink.length) || obj.DocumentLink;
            // console.log(documenttype)
            // var imgtype = ["image/jpg","image/jpeg","image/png","application/pdf"];
            // if(documenttype == "pdf"){

            // }
            // if(documenttype == "jpg" || documenttype == "png" || documenttype == "jpeg"){
        ...
        $('#btnaddgymphase').click(addGymPhase);
        $('#btnupdategymphase').click(updateGymPhase);
        // $('#btndeleteuser').click(deletePosition);
        $('#showtobranch').change(loadBranchroles);
        
    });
    function addGymPhase(){
       
        var site_url = '<?=site_url()?>';
        var gymphasename = $('#gymphasename').val();
        var gymphaseactivitystatus = $(":radio[name='activitystatus']:checked").val();
        var gymphasedeletestatus = $(":radio[name='deletestatus']:checked").val();
        ...
        let formData = new FormData();
            formData.append('gymphasename',gymphasename);
            formData.append('gymphaseactivitystatus',gymphaseactivitystatus);
            formData.append('gymphasedeletestatus',gymphasedeletestatus);
        ...
        if($('#photo').val() == ""){
            ...
            if(jQuery.inArray(document.type, imgtype) !== -1 && document.size < 2048000){
                $.ajax({
                    type:'POST',
                    url:site_url +'masterdata/MasterDataGymPhase/insertMasterDataGymPhaseFromAjax',
                    dataType:"json",
                    data:formData,
                    async: true,
                    cache: false,
    ...
    function updateGymPhase(){
       
       var site_url = '<?=site_url()?>';
        var gymphaseID = $('#gymphaseID').val();
        var site_url = '<?=site_url()?>';
        var gymphasename = $('#gymphasename').val();
        var gymphaseactivitystatus = $(":radio[name='activitystatus']:checked").val();
        var gymphasedeletestatus = $(":radio[name='deletestatus']:checked").val();
        ...
        let formData = new FormData();
            ...
            formData.append('showtobranchrole',showtobranchrole);
            formData.append('haschild',haschild);
            formData.append('gymphaseID',gymphaseID);
        var imgtype = ["image/jpg","image/jpeg","image/png","application/pdf"];
        // console.log(document.type)
        if($('#photo').val() == ""){
            $('.form-group.image img').attr('src', "");
            ...
            if($('#document').val().length > 0 ){
                if(jQuery.inArray(document.type, imgtype) !== -1 && document.size < 2048000){
                    $.ajax({
                        type:'POST',
                        url:site_url +'masterdata/MasterDataGymPhase/updateMasterDataGymPhaseFromAjax',
                        dataType:"json",
                        data:formData,
                        async: true,
                        cache: false,
            ...
            "ajax": {
                ...
                "dataSrc": function (data) {
                    ...
                    for (var i = 0; i < data.length; i++){
                        ...
                        data[i]["edit"] = "<button class='btn btn-info user_action' data-action='edit' data-obj='" + data_array + "' data-toggle='modal' data-target='#GymPhaseModal'><i class='fa fa-edit'></i> Edit</button>";
                        data[i]["delete"] = "<button class='btn btn-danger user_action' data-action='delete' data-obj='" + data_array + "' data-toggle='modal' data-target='#GymPhaseModal'><i class='fa fa-times'></i> Deactivate</button>";
                    }
    ...
    function loadBranchroles(e){
        ...
        $.ajax({
           type:'POST',
           url:site_url +'masterdata/MasterDataGymPhase/loadBranchRoles',
           dataType:"json",
           data:{
    ...
    function loadBranch(e){
        ...
        $.ajax({
            type:'POST',
            url: site_url + 'masterdata/MasterDataGymPhase/loadBranch',
            dataType:"json",
            async: true,
            cache: false,
            contentType: false,
