...
<!-- Left side column. contains the sidebar -->
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        <?=str_replace('_', ' ', $title);?>
        <small>MarketingResourceslist</small>
      </h1>
      <ol class="breadcrumb">
      <li><a href="<?=site_url().''.'Welcome';?>"><i class="fa fa-dashboard"></i>Home</a></li>
        <li><a href="#" data-toggle='modal' data-target='#MarketingResourcesModal'><i class="fa fa-keyboard-o"></i>Register Marketing Resources Information</a></li>      
      </ol>
    </section>
	...
	<div class="box">
            <div class="box-header">
              <h3 class="box-title">Marketing Resources Lists</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <table id="marketingresourcesListTable" class="table table-bordered table-striped">
                <thead>
                <tr>
                    <th width="10">[#]</th>
  ...
  <!-- /.content-wrapper -->
<!-- start modal -->
<div class="modal fade" id="MarketingResourcesModal" role="dialog">
      <div class="modal-dialog modal-sm">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal">&times;</button>
            <h4 class="modal-title">Marketing Resources</h4>
          </div>
          <div class="modal-body">
            <form role="form">
                ...
                <div class="box-body">
                    <input type="hidden" id="marketingresourcesID"/>
                    ...
                    <div class="form-group">
                      <label for="marketingresourcesname">Marketing Resources Name</label>
                      <input type="text" class="form-control" id="marketingresourcesname" placeholder="Enter Marketing Resources Name">
                    </div>
                    <div class="form-group">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-primary" id="btnaddmarketingresources">Submit</button>
            <button type="button" class="btn btn-success" id="btnupdatemarketingresources" style="display:none;">Update</button>
            <button type="button" class="btn btn-danger" id="btndeletemarketingresources" style="display:none;">Delete</button>
...
<script>
    $(document).ready(function(){
        ...
        loadBranch();
        $('#MarketingResourcesModal').on('shown.bs.modal', function (e) {
        action = $(e.relatedTarget).attr('data-action');
        if(action === 'edit') {
                $('#btnupdatemarketingresources').show();
                $('#btndeletemarketingresources').hide();
            }else if(action === 'delete'){
                $('#btnupdatemarketingresources').hide();
                $('#btndeletemarketingresources').hide();
        }else{
            $('#btnaddmarketingresources').show();
            $('#btnupdatemarketingresources').hide();
            $('#btndeletemarketingresources').hide();
        }    
        ...
        $(document).on('click','.user_action',function(){
            ...
            $('#marketingresourcesID').val(obj.SysID);
            $('#marketingresourcesname').val(obj.ResourcesMarketingName);
        ...
        $('#btnaddmarketingresources').click(addMarketingResources);
        $('#btnupdatemarketingresources').click(updateMarketingResources);
        // $('#btndeleteuser').click(deletePosition);
        $('#showtobranch').change(loadBranchroles);
        
    });
    function addMarketingResources(){
       
        var site_url = '<?=site_url()?>';
        var marketingresourcesname = $('#marketingresourcesname').val();
        var marketingresourcesactivitystatus = $(":radio[name='activitystatus']:checked").val();
        var marketingresourcesdeletestatus = $(":radio[name='deletestatus']:checked").val();
        ...
        let formData = new FormData();
            formData.append('resourcesmarketingname',marketingresourcesname);
            formData.append('resourcesmarketingactivitystatus',marketingresourcesactivitystatus);
            formData.append('resourcesmarketingdeletestatus',marketingresourcesdeletestatus);
        ...
        if($('#photo').val() == ""){
            ...
            if(jQuery.inArray(document.type, imgtype) !== -1 && document.size < 2048000){
                $.ajax({
                    type:'POST',
                    url:site_url +'resources/GymResourcesMarketing/insertGymResourcesMarketingFromAjax',
                    dataType:"json",
                    data:formData,
                    async: true,
                    cache: false,
                    ...
                    {
                        if(data.error === true){
                            call_alert_error('MarketingResourcesModal',data.message);
                        }else{
                            loadMarketingResources();
                            call_alert_success('MarketingResourcesModal',data.message,'1');
                        }
                    }
                });
        }else{
            if(jQuery.inArray(document.type, imgtype) === -1){
                call_alert_error('MarketingResourcesModal',"File Type: " + document.type + " is not acceptable");
            }
            if(document.size > 2048000){
                call_alert_error('MarketingResourcesModal',"File Size: " + document.size + " is greater than the acceptable size of 2048KB");
            }
            if(jQuery.inArray(document.type, imgtype) === -1 && document.size > 2048000){
                call_alert_error('MarketingResourcesModal',"File Type: " + document.type + " is not acceptable and File Size: " + document.size + " is greater than the acceptable size of 2048KB");
            }
            }
        }  
    }
    function updateMarketingResources(){
       
       var site_url = '<?=site_url()?>';
        var marketingresourcesID = $('#marketingresourcesID').val();
        var site_url = '<?=site_url()?>';
        var marketingresourcesname = $('#marketingresourcesname').val();
        var marketingresourcesactivitystatus = $(":radio[name='activitystatus']:checked").val();
        var marketingresourcesdeletestatus = $(":radio[name='deletestatus']:checked").val();
        ...
        console.log(marketingresourcesID)
        let formData = new FormData();
            formData.append('resourcesmarketingname',marketingresourcesname);
            formData.append('resourcesmarketingactivitystatus',marketingresourcesactivitystatus);
            formData.append('resourcesmarketingdeletestatus',marketingresourcesdeletestatus);
            ...
            formData.append('showtobranchrole',showtobranchrole);
            formData.append('queryinformation',queryinformation);
            formData.append('marketingresourcesID',marketingresourcesID);
        var imgtype = ["image/jpg","image/jpeg","image/png","application/pdf"];
        // console.log(document.type)
        if($('#photo').val() == ""){
            $('.form-group.image img').attr('src', "");
            ...
            if($('#document').val().length > 0 ){
                if(jQuery.inArray(document.type, imgtype) !== -1 && document.size < 2048000){
                    $.ajax({
                        type:'POST',
                        url:site_url +'resources/GymResourcesMarketing/updateGymResourcesMarketingFromAjax',
                        dataType:"json",
                        data:formData,
                        async: true,
                        cache: false,
    ...
    function loadMarketingResources(){
        ...
        $('#marketingresourcesListTable').DataTable({
            destroy: true,
            lengthMenu: [[10, 20, -1], [10, 20, "All"]],
            "ajax": {
                "url": site_url +"resources/GymResourcesMarketing/ajaxLoadGymResourcesMarketing" ,
                "type": "POST",
                "dataSrc": function (data) {
                    ...
                    for (var i = 0; i < data.length; i++){
                        ...
                        data[i]["edit"] = "<button class='btn btn-info user_action' data-action='edit' data-obj='" + data_array + "' data-toggle='modal' data-target='#MarketingResourcesModal'><i class='fa fa-edit'></i> Edit</button>";
                        data[i]["delete"] = "<button class='btn btn-danger user_action' data-action='delete' data-obj='" + data_array + "' data-toggle='modal' data-target='#MarketingResourcesModal'><i class='fa fa-times'></i> Deactivate</button>";
                    }
    ...
    function loadBranchroles(e){
        ...
        $.ajax({
           type:'POST',
           url:site_url +'resources/GymResourcesMarketing/loadBranchRoles',
           dataType:"json",
           data:{
    ...
    function loadBranch(e){
        ...
        $.ajax({
            type:'POST',
            url: site_url + 'resources/GymResourcesMarketing/loadBranch',
            dataType:"json",
            async: true,
            cache: false,
            contentType: false,
