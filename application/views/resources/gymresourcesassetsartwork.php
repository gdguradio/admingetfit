...
<!-- Left side column. contains the sidebar -->
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        <?=str_replace('_', ' ', $title);?>
        <small>CampaignResourceslist</small>
      </h1>
      <ol class="breadcrumb">
      <li><a href="<?=site_url().''.'Welcome';?>"><i class="fa fa-dashboard"></i>Home</a></li>
        <li><a href="#" data-toggle='modal' data-target='#CampaignResourcesModal'><i class="fa fa-keyboard-o"></i>Register Campaign Resources Information</a></li>      
      </ol>
    </section>
	...
	<div class="box">
            <div class="box-header">
              <h3 class="box-title">Campaign Resources Lists</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <table id="campaignresourcesListTable" class="table table-bordered table-striped">
                <thead>
                <tr>
                    <th width="10">[#]</th>
  ...
  <!-- /.content-wrapper -->
<!-- start modal -->
<div class="modal fade" id="CampaignResourcesModal" role="dialog">
      <div class="modal-dialog modal-sm">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal">&times;</button>
            <h4 class="modal-title">Campaign Resources</h4>
          </div>
          <div class="modal-body">
            <form role="form">
                ...
                <div class="box-body">
                    <input type="hidden" id="campaignresourcesID"/>
                    ...
                    <div class="form-group">
                      <label for="campaignresourcesname">Campaign Resources Name</label>
                      <input type="text" class="form-control" id="campaignresourcesname" placeholder="Enter Campaign Resources Name">
                    </div>
                    <div class="form-group">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-primary" id="btnaddcampaignresources">Submit</button>
            <button type="button" class="btn btn-success" id="btnupdatecampaignresources" style="display:none;">Update</button>
            <button type="button" class="btn btn-danger" id="btndeletecampaignresources" style="display:none;">Delete</button>
...
<script>
    $(document).ready(function(){
        ...
        loadBranch();
        $('#CampaignResourcesModal').on('shown.bs.modal', function (e) {
        action = $(e.relatedTarget).attr('data-action');
        if(action === 'edit') {
                $('#btnupdatecampaignresources').show();
                $('#btndeletecampaignresources').hide();
            }else if(action === 'delete'){
                $('#btnupdatecampaignresources').hide();
                $('#btndeletecampaignresources').hide();
        }else{
            $('#btnaddcampaignresources').show();
            $('#btnupdatecampaignresources').hide();
            $('#btndeletecampaignresources').hide();
        }    
        ...
        $(document).on('click','.user_action',function(){
            ...
            $('#campaignresourcesID').val(obj.SysID);
            $('#campaignresourcesname').val(obj.ResourcesCampaignName);
        ...
        $('#btnaddcampaignresources').click(addCampaignResources);
        $('#btnupdatecampaignresources').click(updateCampaignResources);
        // $('#btndeleteuser').click(deletePosition);
        $('#showtobranch').change(loadBranchroles);
        
    });
    function addCampaignResources(){
       
        var site_url = '<?=site_url()?>';
        var campaignresourcesname = $('#campaignresourcesname').val();
        var campaignresourcesactivitystatus = $(":radio[name='activitystatus']:checked").val();
        var campaignresourcesdeletestatus = $(":radio[name='deletestatus']:checked").val();
        ...
        let formData = new FormData();
            formData.append('resourcescampaignname',campaignresourcesname);
            formData.append('resourcescampaignactivitystatus',campaignresourcesactivitystatus);
            formData.append('resourcescampaigndeletestatus',campaignresourcesdeletestatus);
        ...
        if($('#photo').val() == ""){
            ...
            if(jQuery.inArray(document.type, imgtype) !== -1 && document.size < 2048000){
                $.ajax({
                    type:'POST',
                    url:site_url +'resources/GymResourcesCampaign/insertGymResourcesCampaignFromAjax',
                    dataType:"json",
                    data:formData,
                    async: true,
                    cache: false,
                    ...
                    {
                        if(data.error === true){
                            call_alert_error('CampaignResourcesModal',data.message);
                        }else{
                            loadCampaignResources();
                            call_alert_success('CampaignResourcesModal',data.message,'1');
                        }
                    }
                });
        }else{
            if(jQuery.inArray(document.type, imgtype) === -1){
                call_alert_error('CampaignResourcesModal',"File Type: " + document.type + " is not acceptable");
            }
            if(document.size > 2048000){
                call_alert_error('CampaignResourcesModal',"File Size: " + document.size + " is greater than the acceptable size of 2048KB");
            }
            if(jQuery.inArray(document.type, imgtype) === -1 && document.size > 2048000){
                call_alert_error('CampaignResourcesModal',"File Type: " + document.type + " is not acceptable and File Size: " + document.size + " is greater than the acceptable size of 2048KB");
            }
            }
        }  
    }
    function updateCampaignResources(){
       
       var site_url = '<?=site_url()?>';
        var campaignresourcesID = $('#campaignresourcesID').val();
        var site_url = '<?=site_url()?>';
        var campaignresourcesname = $('#campaignresourcesname').val();
        var campaignresourcesactivitystatus = $(":radio[name='activitystatus']:checked").val();
        var campaignresourcesdeletestatus = $(":radio[name='deletestatus']:checked").val();
        ...
        console.log(campaignresourcesID)
        let formData = new FormData();
            formData.append('resourcescampaignname',campaignresourcesname);
            formData.append('resourcescampaignactivitystatus',campaignresourcesactivitystatus);
            formData.append('resourcescampaigndeletestatus',campaignresourcesdeletestatus);
            ...
            formData.append('showtobranchrole',showtobranchrole);
            formData.append('queryinformation',queryinformation);
            formData.append('campaignresourcesID',campaignresourcesID);
        var imgtype = ["image/jpg","image/jpeg","image/png","application/pdf"];
        // console.log(document.type)
        if($('#photo').val() == ""){
            $('.form-group.image img').attr('src', "");
            ...
            if($('#document').val().length > 0 ){
                if(jQuery.inArray(document.type, imgtype) !== -1 && document.size < 2048000){
                    $.ajax({
                        type:'POST',
                        url:site_url +'resources/GymResourcesCampaign/updateGymResourcesCampaignFromAjax',
                        dataType:"json",
                        data:formData,
                        async: true,
                        cache: false,
    ...
    function loadCampaignResources(){
        ...
        $('#campaignresourcesListTable').DataTable({
            destroy: true,
            lengthMenu: [[10, 20, -1], [10, 20, "All"]],
            "ajax": {
                "url": site_url +"resources/GymResourcesCampaign/ajaxLoadGymResourcesCampaign" ,
                "type": "POST",
                "dataSrc": function (data) {
                    ...
                    for (var i = 0; i < data.length; i++){
                        ...
                        data[i]["edit"] = "<button class='btn btn-info user_action' data-action='edit' data-obj='" + data_array + "' data-toggle='modal' data-target='#CampaignResourcesModal'><i class='fa fa-edit'></i> Edit</button>";
                        data[i]["delete"] = "<button class='btn btn-danger user_action' data-action='delete' data-obj='" + data_array + "' data-toggle='modal' data-target='#CampaignResourcesModal'><i class='fa fa-times'></i> Deactivate</button>";
                    }
    ...
    function loadBranchroles(e){
        ...
        $.ajax({
           type:'POST',
           url:site_url +'resources/GymResourcesCampaign/loadBranchRoles',
           dataType:"json",
           data:{
    ...
    function loadBranch(e){
        ...
        $.ajax({
            type:'POST',
            url: site_url + 'resources/GymResourcesCampaign/loadBranch',
            dataType:"json",
            async: true,
            cache: false,
            contentType: false,
