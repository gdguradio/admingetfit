<?php
class MasterDataGymPhase_ extends CI_Model{
    ...
    // public function loadGymPhase($id=NULL){
    //     if($id!=NULL){
    //         $this->db->where('A.SysID',$id);
    //     }
    //     $this->db->where('PhaseStatus',"yes");
    //     $this->db->where('DeleteStatus',"no");
    //     $query = $this->db->select('A.*')
    ...
    public function getbullenshowto_byid($SysID,$columnname,$roleid = NULL , $branchid= NULL ){
        if($SysID != NULL){
            $this->db->where('A.masterdatagymphasesID',$SysID);
        }
    ...
    public function addGymPhase($details,$mapping){
        $this->db->trans_start();
        $this->db->insert('masterdatagymphases',$details);
        $last_id = $this->db->insert_id();
        $tmp = array();
        foreach($mapping AS $key=>$val){
            $mapping[$key]['masterdatagymphasesID'] = $last_id;
        }
        // print_r($mapping);
        $this->db->insert_batch('masterdatagymphasesshowto',$mapping);
    ...
    public function updateGymPhase($details,$mapping,$id){
        $this->db->trans_start();

        $this->db->where('SysID',$id);
        $this->db->update('masterdatagymphases',$details); 
        foreach($mapping AS $key=>$val){
            $mapping[$key]['masterdatagymphasesID'] = $id;
        }
        $this->db->where('masterdatagymphasesID',$id);
        $this->db->delete('masterdatagymphasesshowto');
        $this->db->insert_batch('masterdatagymphasesshowto',$mapping);
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE){
            return FALSE;
