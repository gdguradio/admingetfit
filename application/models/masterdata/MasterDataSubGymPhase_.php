<?php
class MasterDataSubGymPhase_ extends CI_Model{
    ...
    public function loadSubGymPhase(){
        $this->db->where('SubPhaseStatus',"yes");
        $this->db->where('DeleteStatus',"no");      
        $query = $this->db->select('*')
                ->from('masterdatasubgymphases')
                ->get();
            ...
            if($query->num_rows() > 0){
                ...
                foreach($query->result() as $x){
                    array_push($result,array(
                        ...
                        'SubPhaseStatus'   =>  $x->SubPhaseStatus,
                        'DeleteStatus'   =>  $x->DeleteStatus,
                        'maingymphaseID'   =>  $x->maingymphaseID,
    ...
    public function getbullenshowto_byid($SysID,$columnname,$roleid = NULL , $branchid= NULL ){
        if($SysID != NULL){
            $this->db->where('A.masterdatasubgymphasesID',$SysID);
        }
    ...
    public function addSubGymPhase($details,$mapping){
        $this->db->trans_start();
        $this->db->insert('masterdatasubgymphases',$details);
        $last_id = $this->db->insert_id();
        $tmp = array();
        foreach($mapping AS $key=>$val){
            $mapping[$key]['masterdatasubgymphasesID'] = $last_id;
        }
        // print_r($mapping);
        $this->db->insert_batch('masterdatasubgymphasesshowto',$mapping);
        ...
        // $query = $this->db->insert('masterdatasubgymphases',$data_array);
        // if ($query){
        //     return TRUE;
        // }
    }
    public function updateSubGymPhase($details,$mapping,$id){
        $this->db->trans_start();

        $this->db->where('SysID',$id);
        $this->db->update('masterdatasubgymphases',$details); 
        foreach($mapping AS $key=>$val){
            $mapping[$key]['masterdatasubgymphasesID'] = $id;
        }
        $this->db->where('masterdatasubgymphasesID',$id);
        $this->db->delete('masterdatasubgymphasesshowto');
        $this->db->insert_batch('masterdatasubgymphasesshowto',$mapping);
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE){
            return FALSE;
    ...
    public function loadMainGymPhaseWithSub($haschild){
        if($haschild != NULL){
            $this->db->where('A.HasSub',$haschild);
        }
        $query = $this->db->select("A.*")
    ...
    public function gymphasetype_checker($gymphase_type,$id = NULL){
        $this->db->where('PhaseName',$gymphase_type);
        if($id != NULL){
            $this->db->where('SysID !=',$id);
        }
        $query = $this->db->select('PhaseName')
                ->from('masterdatasubgymphases')
                ->get();
        if($query){
            if($query->num_rows() > 0){
