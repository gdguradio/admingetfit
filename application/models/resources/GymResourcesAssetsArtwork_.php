<?php
class GymResourcesAssetsArtwork_ extends CI_Model{
    ...
    public function loadGymResourcesAssetsArtwork(){
        $this->db->where('ResourcesAssetsArtworkStatus',"yes");
        $this->db->where('DeleteStatus',"no");      
        $query = $this->db->select('*')
                ->from('gymresourcesassetsartwork')
                ->get();
            ...
            if($query->num_rows() > 0){
                ...
                foreach($query->result() as $x){
                    array_push($result,array(
                        ...
                        'ResourcesAssetsArtworkStatus'   =>  $x->ResourcesAssetsArtworkStatus,
    ...
    public function getbullenshowto_byid($SysID,$columnname,$roleid = NULL , $branchid= NULL ){
        if($SysID != NULL){
            $this->db->where('A.gymresourcesassetsartworkID',$SysID);
        }
        ...
        $query = $this->db->select($columnname)
                ->from('gymresourcesassetsartworkshowto AS A')
    ...
    public function addGymResourcesAssetsArtwork($details,$mapping){
        $this->db->trans_start();
        $this->db->insert('gymresourcesassetsartwork',$details);
        $last_id = $this->db->insert_id();
        $tmp = array();
        foreach($mapping AS $key=>$val){
            $mapping[$key]['gymresourcesassetsartworkID'] = $last_id;
        }
        // print_r($mapping);
        $this->db->insert_batch('gymresourcesassetsartworkshowto',$mapping);
    ...
    public function updateGymResourcesAssetsArtwork($details,$mapping,$id){
        $this->db->trans_start();

        $this->db->where('SysID',$id);
        $this->db->update('gymresourcesassetsartwork',$details); 
        foreach($mapping AS $key=>$val){
            $mapping[$key]['gymresourcesassetsartworkID'] = $id;
        }
        $this->db->where('gymresourcesassetsartworkID',$id);
        $this->db->delete('gymresourcesassetsartworkshowto');
        $this->db->insert_batch('gymresourcesassetsartworkshowto',$mapping);
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE){
            return FALSE;
    ...
    public function resourcesassetsartworktype_checker($gymphase_type,$id = NULL){
        $this->db->where('ResourcesAssetsArtworkName',$gymphase_type);
        if($id != NULL){
            $this->db->where('SysID !=',$id);
        }
        $query = $this->db->select('ResourcesAssetsArtworkName')
                ->from('gymresourcesassetsartwork')
                ->get();
        if($query){
            if($query->num_rows() > 0){
