<?php
class GymResourcesCampaign_ extends CI_Model{
    ...
    public function loadGymResourcesCampaign(){
        $this->db->where('ResourcesCampaignStatus',"yes");
        $this->db->where('DeleteStatus',"no");      
        $query = $this->db->select('*')
                ->from('gymresourcescampaign')
                ->get();
            ...
            if($query->num_rows() > 0){
                ...
                foreach($query->result() as $x){
                    array_push($result,array(
                        ...
                        'ResourcesCampaignStatus'   =>  $x->ResourcesCampaignStatus,
    ...
    public function getbullenshowto_byid($SysID,$columnname,$roleid = NULL , $branchid= NULL ){
        if($SysID != NULL){
            $this->db->where('A.gymresourcescampaignID',$SysID);
        }
        ...
        $query = $this->db->select($columnname)
                ->from('gymresourcescampaignshowto AS A')
    ...
    public function addGymResourcesCampaign($details,$mapping){
        $this->db->trans_start();
        $this->db->insert('gymresourcescampaign',$details);
        $last_id = $this->db->insert_id();
        $tmp = array();
        foreach($mapping AS $key=>$val){
            $mapping[$key]['gymresourcescampaignID'] = $last_id;
        }
        // print_r($mapping);
        $this->db->insert_batch('gymresourcescampaignshowto',$mapping);
    ...
    public function updateGymResourcesCampaign($details,$mapping,$id){
        $this->db->trans_start();

        $this->db->where('SysID',$id);
        $this->db->update('gymresourcescampaign',$details); 
        foreach($mapping AS $key=>$val){
            $mapping[$key]['gymresourcescampaignID'] = $id;
        }
        $this->db->where('gymresourcescampaignID',$id);
        $this->db->delete('gymresourcescampaignshowto');
        $this->db->insert_batch('gymresourcescampaignshowto',$mapping);
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE){
            return FALSE;
    ...
    public function resourcescampaigntype_checker($gymphase_type,$id = NULL){
        $this->db->where('ResourcesCampaignName',$gymphase_type);
        if($id != NULL){
            $this->db->where('SysID !=',$id);
        }
        $query = $this->db->select('ResourcesCampaignName')
                ->from('gymresourcescampaign')
                ->get();
        if($query){
            if($query->num_rows() > 0){
