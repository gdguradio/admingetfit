<?php
class FranchiseGymInformation_ extends CI_Model{
    ...
    public function loadFranchiseBranchInformation(){
        $this->db->where('BranchType',"franchise");
        // $query = $this->db->from('Branchdetails')
        $query = $this->db->select('A.*')
                ->from('Branchdetails as A')
                // ->join('gymfranchiselogin as B','A.SysID = B.BranchDetailsID','inner left')
                ->get();
        $result = array();        
        if($query){
            if($query->num_rows() > 0){
    ...
    public function getUserListByID($id){
        ...
        ->join('masterdataposition as B','A.PositionID = B.SysID','inner left')
        ->get();

        // $query = $this->db->from('gymfranchiselogin')
    ...
    public function loadBranch(){
        $this->db->where('BranchType',"franchise");
        $query = $this->db->select('A.*,B.SysID AS franchisegymloginID,A.SysID As franchisebranchID')
                ->from('Branchdetails as A')
                ->join('gymfranchiselogin as B','A.SysID = B.BranchDetailsID','inner left')
                ->get();
        if($query){
